---
name: email-handler
description: Email command center for inbox briefings via outlook-bridge MCP, reply drafts via Outlook, style-matched drafting, and self-learning
---
...
Call `mcp__plugin_mail_outlook-bridge__outlook_auth_check`. If `status != "ok"`, surface to the user:
...
Before drafting, read the style guide at `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md`. If it does not exist (first run), copy `${CLAUDE_PLUGIN_ROOT}/shared/style-guide-example.md` to `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md` and inform the user:

> "No style guide found. I've created a template at `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md`. Drafts will use generic professional defaults until you run `/style-sync` to analyse your sent email corpus and build a personalized guide."
...
1. Search Archive via `mcp__plugin_mail_outlook-bridge__outlook_list_mail` (folder: "Archive", select Id+Subject+From+ReceivedDateTime+ConversationId, filter to messages where `From.EmailAddress.Address` matches the user's own address, compared case-insensitively) for matching sent emails by subject keywords + date. Use `mcp__plugin_mail_outlook-bridge__outlook_get_mail` to retrieve body text once a candidate match is found.
2. Extract actual reply text (strip signature/quoted text)
3. Compare draft vs actual, classifying each on these dimensions:
...
6. Before updating `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md`, save a timestamped backup:
...
7. Update `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md` with corrections
...
1. Read Archive via `mcp__plugin_mail_outlook-bridge__outlook_list_mail` (last 20 messages sent by the user, or since `last_run` timestamp from `inbox-state.json`):
   ...
   Filter client-side to messages where `From.EmailAddress.Address` matches the user's own address, compared case-insensitively. For body content, follow up with `mcp__plugin_mail_outlook-bridge__outlook_get_mail` for each Id.
...
3. Compare against style guide profiles for that recipient in `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md`
...
5. Update `${CLAUDE_PLUGIN_ROOT}/shared/style-guide.md` with statistical evidence:
...
> **Note**: The style guide is statistically grounded from knowledge store analysis. It can be fully rebuilt from the email corpus using `/style-rebuild`, which lives in the optional `mail-pro` plugin in the [`plessas-lab`](https://github.com/weirdapps/plessas-lab) marketplace and requires the private `second-brain` knowledge store.
...
For each message, follow up with `mcp__plugin_mail_outlook-bridge__outlook_get_mail` to retrieve the full body when needed:
...
Use `mcp__second-brain__person_context` with `name_or_email="<sender_name>"`. It returns email history, topics, sentiment, decisions, open actions, and communication pattern in a single call.
...
**Topic/thread context** (what happened before this email):
...
```
mcp__plugin_mail_outlook-bridge__outlook_list_mail   # list with select/since/all/max
mcp__plugin_mail_outlook-bridge__outlook_get_mail    # full body + attachments metadata
mcp__plugin_mail_outlook-bridge__outlook_list_folders, outlook_find_folder, outlook_create_folder, outlook_move_mail
```
...
```
mcp__plugin_mail_outlook-bridge__outlook_send_mail   # draft-first by default; pass send_now:true to dispatch immediately
```

The bridge wraps `outlook-cli send-mail` (Microsoft Graph v2.0). Cross-platform, with no AppleScript dependency. See `commands/send-mail.md` for the full call shape (to/cc/bcc/subject/html_body/attach/no_cc_self/dry_run).

### Replying/Forwarding: text drafts presented for human paste

Although `outlook-bridge` exposes `outlook_reply` / `outlook_reply_all` / `outlook_forward` tools, this agent deliberately presents reply drafts as text in the conversation rather than auto-injecting them. Reasons:
