...
- The marketplace setup script has been run (`installers/install.sh`), which puts `outlook-cli` on your PATH
...
| No tool whose name begins with `mcp__plugin_mail_outlook-bridge__` in your tool list | Re-run the marketplace's `installers/install.sh` to (re)build the bundled MCP server |
