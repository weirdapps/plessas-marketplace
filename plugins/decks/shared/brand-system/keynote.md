...
> Machine source: [`tokens.yaml`](tokens.yaml), section `keynote` (dark palette, gutter). This file
> explains the format and quotes those values; a value changes in tokens.yaml first.
...
and `tools/nbg-presentation/nbg_build.py` (`decks-py build`).

Generator: `tools/nbg-keynote/nbg_keynote.py`, run as `decks-py keynote` (YAML in, PPTX + PDF out).
...
Each token maps to a light-mode colour in `colors.md`. Do not introduce new colours. The hex values
are `tokens.yaml` → `keynote.dark`, which spells the names with underscores (`ink_2`, `accent_bar`)
and calls the ground `ground_from` / `ground_to`. A keynote YAML accepts `ink`, `ink-2`, `ink-3`,
`accent` and `negative` as colour names, or a `#RRGGBB` hex.
...
| Element | Value | Note |
|---|---|---|
| Side gutter | 155 (0.807") both sides (`tokens.yaml` `keynote.gutter_px`) | Wider than the business 0.374" on purpose: read from 20m |
| Safe area | 100 from every edge | Nothing critical outside it |
| Footer row | slide text ends above y=1280 | The logo and source note own the band below |
...
| Footer logo | (155, 1300), height 46, white Greek wordmark | Cover: height 60, bottom-right. Closing: height 52 at (155, 1290). Back: height 150, centred |
| Source note | top (ascender line) at y=1312, 40px right of the logo, Regular 24, `ink-3` | Never smaller than 24. With Aptos the baseline falls near y=1335 |
...
|---|---|---|---|
| Cover title | ExtraBold | 150, at most 2 lines | `ink` |
| Divider title | ExtraBold | 96, line height 1.17× | `ink` |
...
| Bar value / bar category | ExtraBold 60 / Regular 34 (up to 3 bars); ExtraBold 46 / Regular 30 (4 to 7) | | `ink` / `ink-2`, highlighted category `ink` |
...
| Type | Says | Anchors |
|---|---|---|
| `cover` | who is speaking and about what | photo + scrim; title y=300, wrapped inside the gutters; subtitle y=500, one title line lower when the title takes two; speaker block y=1140; logo bottom-right |
...
| `bars` | a comparison across categories | kicker; title y=250; bars baseline y=1170; optional `axis_label`, a one-line unit note at y=1235 (not an axis) |
...
| `divider` | the talk turns here | photo + scrim; kicker; ExtraBold title on the open side (from x=1290 to the gutter, or from the gutter with `align: left`); logo only, no source |
...
2. **Contrast is measured, not eyeballed.** Every text element (kicker, titles, statements,
   values, captions, bar labels, speaker block, source note) is laid out first; the compositor
   then samples the 90th-percentile luminance under each box and deepens a feathered scrim patch
   until the text clears WCAG: 4.5:1 for text under 48px, 3:1 at or above it. Only then is any
   text drawn, so no patch can dim text already on the slide. If a patch cannot get there it
   warns; change the photograph.
3. **Charts are a bounded concession.** Keynote charts are drawn primitives, not native PPTX
   ...
   bars, no axes, no gridlines, no legend.** An `axis_label` is a one-line unit note, not an axis.
   A slide needing a real chart is not a keynote slide.
4. **Bars keep a true zero baseline**, and are capped at 380px wide for every bar count, the group
   centred, so a few bars read as a comparison rather than a wall. Bars take `accent-bar`; with a
   `highlight`, that bar takes `accent` and the rest `accent-mute`. Negative values fail
   `--validate`: state a decline as a `hero-stat` with `color: negative`.
...
7. **JPEG at quality 92, 4:4:4 chroma**, in the frames and in the PDF. 4:2:0 smears the grain and
   the fine type.
8. **Ship the PDF too.** The PDF is what goes on the conference AV laptop.
9. **Text fits, and `--validate` proves it.** Every slide is laid out with the renderer's own fonts
   and wrapping. Text past the right gutter, into the footer row, outside the safe area, a cover
   title over 2 lines, or two blocks that overlap fail the build.
10. **Accessible as a file, not only on stage (Standard #22).** Each PPTX slide carries a title and
    alt text holding everything the slide shows, because the slide itself is one flattened image.
...
Inside Claude Code the launcher is `${CLAUDE_PLUGIN_ROOT}/bin/decks-py`. It builds the tool's
Python environment on first use; see `tools/nbg-keynote/README.md` for fonts and flags. See
`tools/nbg-keynote/example.yaml` for the full YAML surface.
