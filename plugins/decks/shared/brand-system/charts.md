...
| Legend | Bottom, 12pt `#202020`; omit it when the series are labelled directly |
| Bar and column | Gap width 35%; stacked bars overlap 100% |
| Line | 3.5pt, straight segments (no smoothing), hollow circle markers size 6 (white fill, ring in the series colour at 3.5pt) |
...
- Stacked: labels sit inside the segments, so each series' label colour comes from the contrast
  picker (dark on `#00ADBF`, `#BEC1BE`, `#00DFF8`; white on `#003841`, `#007B85`).

## Doughnut Charts

**ALWAYS use doughnut instead of pie charts.** Hole 55%. Label slices with percentages (12pt Bold,
contrast-picked inside a slice), legend at the bottom (12pt `#202020`) or direct labels.

## Line and Area-Line Charts

### Area-Line (the default for a time series)

A line with a low-opacity fill below it: 3.5pt line, straight segments, hollow circle markers,
15% fill in the series colour. No gridlines, no axis lines, muted value-axis labels (11pt
`#939793`), category labels 12pt `#202020`. Data labels are off: the value a reader needs sits in
the KPI callout.
...
- Marker outline: the series colour, at the line width (3.5pt)
...
series.marker.format.line.width = Pt(3.5)                               # match the line
```
