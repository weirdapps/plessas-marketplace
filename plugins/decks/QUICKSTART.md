...
- The `plessas-marketplace` set up (one-time, via `installers/install.sh`, which also builds the three Python venvs the deck tools need)
- Python 3.12 or newer. This is a hard floor: `numpy>=2.5.2` will not resolve on 3.11
...
Other commands: `/presentation-review` (teach the system your style), `/create-keynote` (a stage talk, not a deck), and the creative toolkit: `/create-icon`, `/create-infographic`, `/create-mockup`. Nine in total.
...
- **Validator**: `plugins/decks/tools/nbg-presentation/nbg_validate.py`, which runs automatically as the QA gate; you can also invoke it directly on any PPTX through that tool's `.venv/bin/python3`
- **The agents**: all eight in `plugins/decks/agents/`, which is the only directory the plugin loader scans
