...
if [ ! -d "$DIR/node_modules" ]; then
  ...
  {
    echo "teams-bridge: FATAL: dependencies are not installed, so the server cannot start."
...
if [ ! -f "$DIR/dist/server.js" ]; then
  ...
  {
    echo "teams-bridge: FATAL: the server is not built, so there is nothing to run."
