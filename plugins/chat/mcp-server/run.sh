...
NODE_BIN="$(find_node)" || {
  ...
  echo "teams-bridge: FATAL: node binary not found. Install Node.js v20+ from https://nodejs.org/" >&2
  exit 1
}
...
if [ "$NODE_MAJOR" -lt 20 ]; then
  ...
  echo "teams-bridge: FATAL: node $BAD_NODE is below the required v20. Install Node.js v20+ from https://nodejs.org/" >&2
  exit 1
fi
...
if [ ! -d "$DIR/node_modules" ]; then
  ...
  {
    echo "teams-bridge: FATAL: bundle/server.mjs is missing AND dependencies are not installed, so the server cannot start."
...
if [ ! -f "$DIR/dist/server.js" ]; then
  ...
  {
    echo "teams-bridge: FATAL: bundle/server.mjs is missing and dist/ is not built, so there is nothing to run."
