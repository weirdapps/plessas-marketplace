...
function buildSuggestion(opts: {
  ...
  if (opts.cliMode === 'path') {
    return `teams-cli not bundled in mcp-server/node_modules. The MCP is using a global teams-cli on PATH. To get the bundled (more robust) install, run: cd ${SERVER_ROOT} && npm install`;
  }
