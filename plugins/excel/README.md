...
For best results, install Anthropic's `document-skills` plugin (`/plugin install document-skills`). Without it, the commands fall back to `openpyxl`/`pandas` (auto-installed on first use).
