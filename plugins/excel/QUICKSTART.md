...
Reads modern Excel files (`.xlsx`, `.xlsm`) plus `.csv` and `.tsv`, and runs analysis tuned for executive use. Summarises structure and KPIs, builds pivot tables from a natural-language description, runs period-over-period variance with the periods auto-detected, and prepares a deck brief for the `decks` plugin to turn into a NBG-branded presentation. NBG-shaped defaults throughout: EUR currency, fiscal-year orientation (January-December), Aptos styling on any generated output. Uses `document-skills:xlsx` when available for best results, falls back to `openpyxl`/`pandas` (auto-installed on first use if missing).
...
Bonus: `/excel-to-deck <file>` extracts the headline insights, writes a deck brief to `~/Downloads/`, and tells you the `decks` command to run next.
...
| Pivot output looks wrong | The pivot is interpreted from your description and computed without a confirmation step. Restate it more explicitly and re-run, e.g. "group by Branch (column F), not by Region (column C)". |
