...
# Two measured occurrences in this tree, both classified, neither a person.
# They are excluded as exact two-word VALUES, the same convention PLACEHOLDER
# above uses, and never as a single word or a path. A one-word exclusion such
# as a bare surname-shaped token would hand anyone a suppression token, which
# is precisely the defect the old "copyright" filter had.
#
#   ΕΘΝΙΚΗ ΤΡΑΠΕΖΑ  x2  plugins/decks/shared/presentation-style-guide.md:103,425
#                       the Greek logo caption. An organisation rather than a
#                       person, and this repo opts out of the employer-name
#                       check on purpose via .pii-gauntlet-allow-employer-name.
#   ΑΓΙΟΣ ΙΩΑΝΝΗΣ   x1  plugins/decks/tools/nbg-keynote/test_nbg_keynote.py:22
#                       a placename, and the input that proves greek_upper()
#                       strips the tonos. It is exactly name-shaped: no length
#                       threshold and no morphology rule separates it from a
#                       real person, so it is named here rather than pretended
#                       away by weakening the rule for everybody.
#
# The residual risk is line-level, because apply_exclusion drops whole lines: a
# real name sharing a line with one of these two phrases would be suppressed.
# Both live on lines that will not acquire one.
NOT_A_PERSON='ΕΘΝΙΚΗ ΤΡΑΠΕΖΑ|ΑΓΙΟΣ ΙΩΑΝΝΗΣ'
check_shape "All-caps Greek personal name" "$GREEK_NAME_SHAPE" "$NOT_A_PERSON"
